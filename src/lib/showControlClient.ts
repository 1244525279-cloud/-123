...
type ShowControlClient = {
  publishState: (patch: Record<string, unknown>) => void;
  publishAudioFrame: (frame: AudioFrameMessage) => void;
  postState: (patch: Record<string, unknown>) => Promise<void>;
  close: () => void;
};
...
function createFirebaseClient(options: ClientOptions): ShowControlClient {
